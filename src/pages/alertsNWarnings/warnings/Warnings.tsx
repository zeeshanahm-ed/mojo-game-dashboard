...
//Hooks and Helpers
import useDeleteWarnings from "./hooks/useDeleteWarnings";
...
import useUpdateWarning from "./hooks/useUpdateWarning";
...
function Warnings({ isLoading, warningData, refetch, setListing, listing }: any) {
    const navigate = useNavigate();
    const [filterListing] = useState<any>({ ...intitialParams });
    const { clientData } = useClientData(filterListing);

    const { setTitle } = useHeaderProps();
    const { deleteWarningsMutate } = useDeleteWarnings();
    ...
    useEffect(() => {
        handleUpdateWarning();
        const modifyData = warningData?.data?.map((data: any) => {
            return {
                ...data,
                checked: false,
            }
    ...
    const handleUpdateWarning = () => {
        const unreadAlerts = warningData?.data?.filter((item: any) => !item?.isRead);
    ...
    const handleDeleteWarnings = () => {
        ...
        deleteWarningsMutate(checkedIdsPayload, {
            onSuccess: () => {
    ...
    const handleDateChange = (date: any) => {
        if (date) {
            setListing((prev: any) => ({ ...prev, warningDate: date?.toISOString() }));
        } else {
            setListing((prev: any) => ({ ...prev, warningDate: null }));
        }
    }
    ...
    const handleCheckbox = (value: any) => {
        const modifyData = state.modifyAlertData.map(alert => {
    ...
    return (
        <section className="overflow-hidden mb-10">
            <div className="flex justify-between items-center mt-10 flex-wrap gap-6">
                <div className="flex justify-between items-center gap-x-6 gap-y-2" >
                    <div className="flex items-center border border-border-gray px-3 h-11 custom-radius min-w-60">
                        ...
                        <DatePicker
                            ...
                            showTime={false}
                            value={listing?.warningDate ? dayjs(listing?.warningDate) : null}
                ...
                <div>
                    <Popconfirm
                        title='Are you sure you want to delete the selected warnings?'
                        onConfirm={handleDeleteWarnings}
                        okText='Yes'
                        cancelText='No'
                    >
                        <Button disabled={handleDisabled()} variant='text' className='border border-danger text-danger font-normal shadow-none h-11 px-2 gap-6 text-sm w-50 custom-radius'><DeleteIcon /> Delete Warnings</Button>
                    </Popconfirm>
                </div>
            </div>
            <CustomAlertTable
            ...
            {!isLoading && <div className="flex justify-center mt-6">
                <Pagination
                    current={warningData?.currentPage}
                    pageSize={warningData?.pageSize}
                    total={warningData?.totalItems}
